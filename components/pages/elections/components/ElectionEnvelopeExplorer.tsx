...
export const EnvelopeExplorer = ({ electionId }: EnvelopeExplorerProps) => {
  ...
  return (
    <Card>
      ...
      <Grid>
        <If condition={!loading && envelopeRange != null}>
          <Then>
            {envelopeRange?.votes.map((envelope, idx) => (
              <EnvelopeCard envelope={envelope} idx={votePage * ENVELOPES_PER_PAGE + idx + 1} key={idx}></EnvelopeCard>
            ))}
          </Then>
          <Else>{renderCardSkeleton(ENVELOPES_PER_PAGE)}</Else>
        </If>
      </Grid>
